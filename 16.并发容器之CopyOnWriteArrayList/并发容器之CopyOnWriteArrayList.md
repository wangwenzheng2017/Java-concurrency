...
回到业务场景中，有很多业务往往是**读多写少**的，

1. 比如**系统配置的信息**，除了在初始进行系统配置的时候需要写入数据，其他大部分时刻其他模块之后对系统信息只需要进行读取，

2. 又比如**白名单**，**黑名单**等配置，只需要读取名单配置然后检测当前用户是否在该配置范围以内。
...
> [volatile对数组的引用](https://blog.csdn.net/mutou_ren/article/details/107530756)
